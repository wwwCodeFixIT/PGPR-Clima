    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <p className="font-semibold text-sm">PGPR Instalacje Sp. z o.o.</p>
        </div>
      </header>
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { label: 'Ekonomiczny',   price: '3 490', device: 'Gree Fairy 3,5 kW',              warranty: '24 mies.', highlight: false },
              { label: 'Rekomendowany', price: '4 790', device: 'Daikin FTXM35R 3,5 kW',          warranty: '36 mies.', highlight: true },
              { label: 'Premium',       price: '6 890', device: 'Mitsubishi Electric MSZ-EF35',    warranty: '60 mies.', highlight: false },
            ].map(({ label, price, device, warranty, highlight }) => (
              <div key={label} className={`relative bg-white dark:bg-gray-900 rounded-2xl border p-6 ${highlight ? 'border-pgpr-blue-600 ring-2 ring-pgpr-blue-600' : 'border-gray-200 dark:border-gray-800'}`}>
                {highlight && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-pgpr-blue-700 text-white text-xs font-semibold px-3 py-1 rounded-full">Polecamy</span>}
