...
import {
  Bell, Search, Menu, ChevronRight, LogOut, Settings, User, Moon, Sun, Monitor,
} from 'lucide-react'
...
const ROUTE_LABELS: Record<string, string> = {
  ...
  reports: 'Raporty', automations: 'Automatyzacje', integrations: 'Integracje',
  settings: 'Ustawienia', new: 'Nowy', edit: 'Edycja',
}
      ...
      {crumbs.map((crumb, i) => (
        <React.Fragment key={crumb.href}>
          ...
          {crumb.isLast ? (
            ...
            <a
              href={crumb.href}
              className="text-muted-foreground hover:text-foreground transition-colors duration-fast truncate max-w-[120px]"
            >
              {crumb.label}
            </a>
          )}
    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content align="end" sideOffset={6}
          className="z-dropdown min-w-[130px] rounded-lg border border-border bg-popover p-1 shadow-floating text-sm text-popover-foreground animate-slide-down-fade">
          {items.map(({ label, value, icon: Icon }) => (
            <DropdownMenu.Item key={value} onSelect={() => setTheme(value)}
              className={cn('flex items-center gap-2.5 rounded-md px-2.5 py-2 cursor-default outline-none select-none transition-colors hover:bg-secondary', theme === value && 'bg-secondary font-medium')}>
...
function NotificationBell({ count = 0 }: { count?: number }) {
  return (
    <Button variant="ghost" size="icon"
      aria-label={count > 0 ? `${count} powiadomień` : 'Powiadomienia'}
      className="relative">
...
function UserMenu({ name, email, role }: { name: string; email: string; role: string }) {
  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button
          className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-secondary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          aria-label={`Menu użytkownika: ${name}`}>
          <Avatar.Root className="h-7 w-7 rounded-full">
            <Avatar.Fallback className="h-7 w-7 rounded-full bg-primary text-primary-foreground text-xs font-semibold flex items-center justify-center">
              {initials(name)}
            </Avatar.Fallback>
          </Avatar.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content align="end" sideOffset={6}
          className="z-dropdown w-56 rounded-lg border border-border bg-popover p-1 shadow-floating text-sm text-popover-foreground animate-slide-down-fade">
          <div className="px-2.5 py-2 border-b border-border mb-1">
            <p className="font-medium text-foreground truncate">{name}</p>
            <p className="text-xs text-muted-foreground truncate">{email}</p>
          </div>
          {[
            { label: 'Mój profil', icon: User,     href: '/app/settings/profile' },
            { label: 'Ustawienia', icon: Settings, href: '/app/settings' },
          ].map(({ label, icon: Icon, href }) => (
            <DropdownMenu.Item key={href} onSelect={() => router.push(href)}
              className="flex items-center gap-2.5 rounded-md px-2.5 py-2 cursor-default outline-none select-none transition-colors hover:bg-secondary">
          ...
          <DropdownMenu.Item onSelect={() => router.push('/api/auth/signout')}
            className="flex items-center gap-2.5 rounded-md px-2.5 py-2 cursor-default outline-none select-none transition-colors hover:bg-destructive/10 hover:text-destructive text-destructive/80">
...
// Fix: user?: T | undefined for exactOptionalPropertyTypes
  ...
  return (
    <header className="flex items-center h-topbar px-4 gap-3 border-b border-border bg-card/80 backdrop-blur-sm shrink-0 sticky top-0 z-sticky">
      <Button variant="ghost" size="icon" className="lg:hidden shrink-0"
        onClick={onMenuClick} aria-label="Otwórz menu">
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {user != null ? (
          ...
          <div className="h-7 w-7 rounded-full bg-muted animate-skeleton-shimmer" aria-hidden="true" />
        )}
      </div>
    </header>
  )
