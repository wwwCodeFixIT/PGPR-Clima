...
// Demo KPI data — replace with real DB queries
const KPI = [
  ...
  { label: 'Urządzenia',           value: '387',     accent: undefined,     icon: <Monitor       className="h-5 w-5" /> },
  { label: 'Czynnik wydany',       value: '48,2 kg', accent: undefined,     icon: <Wind          className="h-5 w-5" /> },
  ...
  return (
    <div className="p-6 space-y-8 max-w-[1600px] mx-auto">
      ...
      <section aria-label="Wskaźniki KPI">
        <h2 className="sr-only">Wskaźniki KPI</h2>
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {KPI.map((kpi) => (
            <KpiCard
              ...
              label={kpi.label}
              value={kpi.value}
              accent={kpi.accent as 'primary' | 'success' | 'warning' | 'destructive' | 'accent' | undefined}
              icon={kpi.icon}
              trendValue={'trendValue' in kpi ? kpi.trendValue : undefined}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Today plan */}
        <section className="lg:col-span-2 rounded-lg border border-border bg-card p-5" aria-label="Plan dnia">
          <h2 className="text-sm font-semibold text-foreground mb-4">Dzisiejszy plan</h2>
          <div className="space-y-3">
            {['08:00 – Montaż klimatyzatora, ul. Kwiatowa 12, Wrocław', '11:30 – Serwis agregatu, Centrum Logistyczne Modlnica', '14:00 – Przegląd VRF, Biurowiec Nova Park', '16:30 – Kontrola szczelności, Hotel Mercure'].map((item) => (
              <div key={item} className="flex items-start gap-3 p-3 rounded-md bg-secondary/50 hover:bg-secondary transition-colors">
        ...
        {/* Alerts */}
        <section className="rounded-lg border border-border bg-card p-5" aria-label="Wymagają uwagi">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: 'Magazyn: R410A poniżej minimum', color: 'bg-warning' },
              { label: 'Faktura #FV/2024/0087 przeterminowana', color: 'bg-destructive' },
            ].map(({ label, color }) => (
              <div key={label} className="flex items-start gap-3">
                <div className={`h-2 w-2 rounded-full ${color} mt-1.5 shrink-0`} aria-hidden="true" />
      ...
      {/* Skeleton preview for upcoming widgets */}
      <section aria-label="Ostatnia aktywność — ładowanie">
